...
*Not imported variables: V1, X1, Herstellernummer, Fehlerhaft_Fahrleistung*
...
*Imported variables: ID_X.x, Produktionsdatum.x, Fehlerhaft.x, Fehlerhaft_Datum.x, ID_X.y, Produktionsdatum.y, Fehlerhaft.y, Fehlerhaft_Datum.y*

*Not imported variables: V1, X1, Herstellernummer.x, Fehlerhaft_Fahrleistung.x, Herstellernummer.y, Fehlerhaft_Fahrleistung.y, Werksnummer.x, Werksnummer.y*
...
# K4
 component_K4 <- fread("Data/Komponente/Komponente_K4.csv", header = TRUE, data.table = FALSE) %>% 
  ...
  mutate(production_date = if_else(is.na(Produktionsdatum.x), Produktionsdatum.y, Produktionsdatum.x)) %>% 
  mutate(failure_date = if_else(is.na(Fehlerhaft_Datum.x), Fehlerhaft_Datum.y, Fehlerhaft_Datum.x)) %>% 
  select(-(1:16)) %>% 
  relocate(id_body, production_date, component_failure, failure_date)  
...
component_K3AG1 <- fread("Data/Komponente/Komponente_K3AG1.csv", header = TRUE, data.table = FALSE) %>% filter(Fehlerhaft.x == 1 | Fehlerhaft.y == 1 | Fehlerhaft == 1) %>% mutate(component_failure = 1) %>% 
  ...
  select(-(1:23)) %>% relocate(id_gearshift, production_date, component_failure, failure_date)  
...
component_K3SG1 <- fread("Data/Komponente/Komponente_K3SG1.csv", header = TRUE, data.table = FALSE) %>% 
  ...
  select(-(1:16)) %>% 
  relocate(id_gearshift, production_date, component_failure, failure_date)  
...
component_K2LE1 <- fread(text = txt, sep = ",", data.table = FALSE) %>% 
  ...
  mutate(failure_date = if_else(!is.na(Fehlerhaft_Datum.x), Fehlerhaft_Datum.x, Fehlerhaft_Datum.y)) %>% 
  select(-(1:16)) %>% 
  relocate(id_seats, production_date, component_failure, failure_date) 
rm(txt)

# K2ST1 --> TXT + No header
component_K2ST1 <- fread("Data/Komponente/Komponente_K2ST1.txt", data.table = FALSE) %>% 
  filter(Fehlerhaft == 1) %>% 
  rename(id_seats = ID_Sitze, production_date = Produktionsdatum, component_failure = Fehlerhaft, failure_date = Fehlerhaft_Datum) %>% 
  relocate(id_seats, production_date, component_failure, failure_date) %>% 
  select(-(5:9))  
...
component_K1BE1 <- fread("Data/Komponente/Komponente_K1BE1.csv", header = TRUE, data.table = FALSE) %>% 
  filter(Fehlerhaft == 1) %>% 
  rename(id_engine = ID_Motor, production_date = Produktionsdatum_Origin_01011970, component_failure = Fehlerhaft, failure_date = Fehlerhaft_Datum) %>% 
  relocate(id_engine, production_date, component_failure, failure_date) %>% 
...
component_K1DI1 <- fread("Data/Komponente/Komponente_K1DI1.csv", header = TRUE, data.table = FALSE) %>% filter(Fehlerhaft.x == 1 | Fehlerhaft.y == 1 | Fehlerhaft == 1) %>% mutate(component_failure = 1) %>% 
  ...
  mutate(failure_date = 
     if_else(!is.na(Fehlerhaft_Datum.x), Fehlerhaft_Datum.x, 
     if_else(!is.na(Fehlerhaft_Datum.y), Fehlerhaft_Datum.y, 
     if_else(!is.na(Fehlerhaft_Datum), Fehlerhaft_Datum, NA)))) %>% relocate(id_engine, production_date, component_failure, failure_date) %>% 
  select(-(5:27)) 
...
txt <- gsub(" ","\n",txt, fixed = TRUE)
part_T01 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
...
txt <- gsub("  ",",",txt, fixed = TRUE)
txt <- gsub("\t","\n",txt, fixed = TRUE)
part_T02 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
...
txt <- gsub("|",",",txt, fixed = TRUE)
txt <- gsub("\v","\n",txt, fixed = TRUE)
part_T03 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
...
txt <- gsub("\t",",",txt, fixed = TRUE)
txt <- gsub("\f","\n",txt, fixed = TRUE)
part_T11 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
...
txt <- gsub("  ",",",txt, fixed = TRUE)
txt <- gsub("\f","\n",txt, fixed = TRUE)
part_T24 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
...
txt <- gsub("  ",",",txt, fixed = TRUE)
txt <- gsub("\b","\n",txt, fixed = TRUE)
part_T31 <- fread(text = txt, sep = ",", nrows = 50, data.table = FALSE)
rm(txt)
