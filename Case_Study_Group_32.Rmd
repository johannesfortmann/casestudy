...
*Note: For identifying the types of components installed in the respective vehicles new columns are added to the two imported dataframes (e.g. components_in_vehicle_type11$Engine_component_type). The component types were identified using a combination of the substring() and the regexpr() function. Afterwards all types of components installed in vehicle type 11 and/or type 12 are collected in an auxillary variable called 'components'.*
...
All datasets of individual parts were cleaned using similar operations as above. It is worth mentioning that columns within multiple dataframes of individual parts had to be merged using the combination of if_else() and is.na() statements.   
...
```{r, message=FALSE, warning = FALSE, cache = TRUE}
# Exporting the final dataframe as csv. dataset
