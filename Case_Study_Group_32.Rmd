...
The goal of this chapter is to present and explain the final dataset. In order to provide the maximum of transparency and to avoid any misunderstandings as well as ambiguities the difference between a **vehicle failure** and a **field-failure** as well as all of the corresponding columns are comprehensively explained:    

* **Vehicle failure ($vehicle_failure)**: is of the datatype factor and has two levels: `r levels(final_data$vehicle_failure)`. Vehicles which are not marked as defective are denoted with `r levels(final_data$vehicle_failure)[1]` whereas vehicles which are marked as defective are denoted with `r levels(final_data$vehicle_failure)[2]`. This column **does not** directly indicate whether vehicles are a field-failure or not. Vehicles which have a 0 in this column can still include part(s) and/or component(s) which are marked defective and are therefore a field-failure. We did include this column in the final dataset in order to distinguish between minor failures such as a component or part failure and an severe failure resulting in the entire vehicle being marked as defective. 
* **Vehicle Failure Date ($vehicle_failure_date)**: contains the data when the respective vehicle was marked as defective. This column **does not** contain the date when a vehicle became a field-failure. 
* **Vehicle Lifespan ($vehicle_lifespan)**: contains the timespan [in days] between the production date and the vehicle failure date. We consider this timespan to be the lifespan of a vehicle. 

* **Field-Failure ($field_failure)**: is of the datatype factor and has two levels: `r levels(final_data$field_failure)`. If marked as `r levels(final_data$field_failure)[2]` the respective vehicle is a field-failure. **In this case the vehicle itself and/or at least one built-in component and/or at least one built-in part were marked as defective in the original datasets**.
* **Earliest Failure Date ($earliest_failure_date)**: if a vehicle is a field-failure, this column contains the date of the first failure which occurred. In other words if multiple failures (e.g. overall two components and three parts have failed) occurred in the same vehicle, this column contains only the date of the earliest failure. 
* **Time till first Failure ($time_till_first_failure)**: contains the timespan [in days] between the production date and the earliest failure date. In other words, this column contains the number of days it took until a vehicle became a field-failure.  

Overall the "final_data" dataframe has `r nrow(final_data)` rows and `r ncol(final_data)` columns. In order to drastically reduce the computing time we present a randomly selected sample (n = 200) of our final dataframe using the *datatable()* function from the "DT" package down below:   

*Note: The randomly selected sample was created using dplyr's sample_n() function.*

*Note: For reasons of readableness and clarity the dataframe's column names were slightly changed using the function's  caption argument (e.g. "id_vehicle" --> "Vehicle ID").*

```{r, message=FALSE, warning = FALSE, cache = TRUE}
# Overview over the final dataframe 
datatable(final_data %>% sample_n(200), caption = "Randomly selected Sample of the Final Dataset (n = 200)", colnames = c("Vehicle ID" = "id_vehicle", "Vehicle Type" = "vehicle_type", "Manufacturing Plant" = "plant", "Location" = "location", "Latitude" = "latitude", "Longitude" = "longitude", "Production date" = "vehicle_production_date", "Vehicle Failure" = "vehicle_failure", "Vehicle Failure Date" = "vehicle_failure_date", "Vehicle Lifespan" = "vehicle_lifespan", "Field-Failure" = "field_failure", "Earliest Failure Date" = "earliest_failure_date", "Time till first Failure" = "time_till_first_failure"))
```


### Export 

In order to incorporate the "final_data" dataframe into our shiny application, the dataframe is exported as .csv file using the *fwrite()* function from the "data.table" package.  
