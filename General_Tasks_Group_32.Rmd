...
Danach werden die beiden Datensätze eingelesen.
```{r }
...
Das Datumsformat aus Logistikverzug_K7 wird ins gleiche Format umgewandelt wie jenes aus Komponente_K7, damit sie subtrahieren kann.
...
#rm(IDNummer)
```
Um sich einen Überblick über die Verteilung zu verschaffen, wird zunächst der Logistikverzug geplotet.

```{r}
hist(Logistikverzug$AdjustierterVerzug, breaks=12, main="Histogram of Logistikverzug", xlab="Logistikverzug (days)")
```


Der Plot weist nicht auf eine Normalverteilung hin, dies wird nun noch mit einem statistischen Test überprüft.
Es muss einer gewählt werden, der auch für große Stichprobenmengen geeignet ist, hier: Anderson-Darling-Test.
```{r}
# Variable auswählen
...
ad.test(Logistikverzug$AdjustierterVerzug)
```
...
```
Da keine Normalverteilung vorliegt und der Plot den Verdacht der Schiefe erregt, wird nun auf diese geprüft.
```{r}
skewness(var)
```
Die Schiefe (Skewness) ist eine statistische Maßzahl, die die Asymmetrie einer Verteilung beschreibt. Ein Wert von 0 bedeutet, dass die Verteilung symmetrisch ist. Wenn der Wert größer als 0 ist, dann deutet dies auf eine rechtsschiefe Verteilung hin, d.h. die Verteilung hat einen längeren rechten Schwanz im Vergleich zum linken Schwanz. Wenn der Wert kleiner als 0 ist, dann deutet dies auf eine linksschiefe Verteilung hin, d.h. die Verteilung hat einen längeren linken Schwanz im Vergleich zum rechten Schwanz.

In diesem Fall ergab die Skewness den Wert von 0.55, was darauf hindeutet, dass die Verteilung wahrscheinlich rechtsschief ist. Dies bedeutet, dass die meisten Datenwerte näher am linken Ende der Verteilung liegen und es einige wenige Ausreißer auf der rechten Seite gibt, die zu einem längeren rechten Schwanz führen.

So eine Verteilung lässt sich sehr gut mit einem Densityplot darstellen, da man die Schiefe dort gut erkennen kann. Die Ausreißer ganz rechts werden damit nicht dargestellt, die Häufigkeit ist allerdings auch so gering, dass sie vernachlässigbar sind.
```{r}
